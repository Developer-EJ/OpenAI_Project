export default function PartyPanel({
  ...
  const currentAreaMeta = areaMeta[currentArea] || areaMeta[defaultAreaId];
  const canCreateParty = currentArea !== defaultAreaId;

  function formatPartyCount() {
    if (!parties.length) {
      ...
      <div className="party-header-card">
        <div className="party-header-row">
          <div className="party-header-copy">
            ...
            <h2>{collapsed ? "AREA" : currentAreaMeta.label}</h2>
            {!collapsed ? <p>{currentAreaMeta.description}</p> : null}
          </div>
        </div>
      </div>
        ...
        {areaOrder.map((areaId) => {
          ...
          return (
            ...
            >
              ...
              {collapsed ? null : <small>{areaId === defaultAreaId ? "허브" : "파티존"}</small>}
            </button>
          );
        })}
      </div>
      ...
      {collapsed ? null : (
        <div className="party-board">
          <div className="party-board-header">
            <div>
              <h3>{currentAreaMeta.label} 파티 보드</h3>
              <p>{formatPartyCount()}</p>
            </div>
